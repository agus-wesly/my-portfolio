...
const Skill = ({ skillRef }) => {
  ...
  const { ref: vueRef, entry: vueEntry } = useInView()
  ...
  return (
    <div className="mb-[5rem]">
      <div ref={skillRef} className="w-full flex flex-col gap-7 items-center">
        <h1 className="text-center text-lg lg:text-[32px] text-white font-extrabold relative self-center after:content-[''] after:w-full after:absolute after:h-1 after:bg-secondary after:-bottom-1 after:left-0 mb-3">
          Skills
        </h1>
      </div>
      <div className="block lg:grid grid-cols-3 gap-8 lg:mt-7">
        {skillItems.map((skill, i) => {
          return (
            <SkillCard
              entry={refArr[i].entry}
              reff={refArr[i].ref}
              key={i}
              idx={i}
              icon={skill.icon}
