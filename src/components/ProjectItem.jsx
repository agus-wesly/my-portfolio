...
const ProjectItem = ({ img, name, desc, url, reff, entry, idx, descSm }) => {
  ...
  return (
    <div
      className={`flex flex-col gap-5 my-5 w-full h-full  opacity-0 lg:mt-7 lg:items-center lg:gap-0 lg:my-8 ${
    ...
    >
      <img
        ...
        src={img}
        alt="project"
        className="rounded-[3px] opacity-[0.9] object-contain mix-blend-exclusion lg:w-[600px] lg:aspect-video"
      />
      <h3 className="text-md font-bold leading-[7px] text-whiteblue lg:hidden">
        {name}
      </h3>
      ...
      <div className="lg:hidden">
        <a
          href={url}
          target="__blank"
          className="flex items-center py-1 gap-2 text-secondary"
        >
      ...
      <div
        className={`hidden lg:flex flex-col gap-4 bg-darkblue p-5 relative after:content-[''] after:w-full after:h-full after:bg-secondary ${
          even
            ? "after:right-0 after:translate-x-2 justify-end text-end"
            : "text-start justify-start after:left-0 after:-translate-x-2"
